//odule Day08_Registers
    open System.Text.RegularExpressions

    type Input = Register of string | Value of int
    type Operator = Inc | Dec
    type Comparer = NotEqual | Equal | GreaterThan | GreaterThanOrEqual | LessThan | LessThanOrEqual
    type Comparison = (Map<string,int> -> bool)
    type Action = (Map<string,int> -> Map<string,int>)
    type Instruction = {register:string;action:Action;comparison:Comparison}

    let getRegister (register:string) (map:Map<string,int>) = 
    ...
    let getValue input map =
        match input with 
        | Value (i) -> i
        | Register (r) -> getRegister r map

    let inc (a:string) (b:int) (map:Map<string,int>) =
        let newValue = (getRegister a map) + b
        Map.add a newValue map

    let dec (a:string) (b:int) (map:Map<string,int>) =
        let newValue = if Map.containsKey a map then map.[a] - b else -b
        Map.add a newValue map
    
    let parseInput a =
        let (success,value) = System.Int32.TryParse a
        if success then (Value value) else (Register a)
    ...
    let compare (p:Comparer) (a:Input) (b:Input) (map:Map<string,int>) =
    ...
    let parse (list,map) line =
        ...
        if match'.Success then
            let r = match'.Groups.[1].Value
            
            let actionType = match'.Groups.[2].Value
            let actionValue = match'.Groups.[3].Value |> int
            let action = if actionType = "inc" then inc else dec

            let a = match'.Groups.[4].Value |> parseInput
            let op = match'.Groups.[5].Value |> parseComparer
            let b = match'.Groups.[6].Value |> parseInput

            let instruction = {register=r;action=(fun map -> action r actionValue map);comparison=(fun map -> compare op a b map)}
            let newList = List.append list [Some instruction]
            if Map.containsKey r map then 
                (newList,Map.add r 0 map)
            else
                (newList,map)
        else 
    ...
    let processLines lines =
        ...
        let rec processRec map (instructions:Instruction option list) =
        ...
        let map' = processRec map list
        printf "Resulting map %A" map'
        Map.toSeq map' |> Seq.map (fun (k,v) -> v) |> Seq.max

    let testCases = [
        "b inc 5 if a > 1",{register="b";action=(fun map -> inc "b" 5 map);comparison=(fun map -> compare GreaterThan (Register "a") (Value 1) map)}
        "a inc 1 if b < 5",{register="a";action=(fun map -> inc "a" 1 map);comparison=(fun map -> compare LessThan (Register "b") (Value 5) map)}
        "c dec -10 if a >= 1",{register="c";action=(fun map -> dec "c" -10 map);comparison=(fun map -> compare GreaterThanOrEqual (Register "a") (Value 1) map)}
        "c inc -20 if c == 10",{register="c";action=(fun map -> inc "c" -20 map);comparison=(fun map -> compare GreaterThanOrEqual (Register "c") (Value 10) map)}
    ]
    ...
    //test (processLines) 1 (actualInput);; // 2757 is too low
